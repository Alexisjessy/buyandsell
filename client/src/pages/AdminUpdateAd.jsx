...
import { useGetUserIdQuery, useGetProductQuery,   useUpdateAdminAdMutation, useDeleteImageMutation } from '../features/api/apiSlice';
...
export default function AdminUpdateAd() {
  ...
  const [updateAd, refetch] =   useUpdateAdminAdMutation();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await userData; // Attend que la requête de l'userId soit terminée
...
useEffect(() => {
...
}, [isUserDataLoaded, isAnnonceDataLoaded, userData, annonce, user.admin, navigate]);
  ...
  const confirmDeleteImage = async () => {
    try {
      ...
      setSuccessMessage(['Image has been deleted.']);
    } catch (error) {
      console.error('Error deleting old image on server side:', error);
    }
  };
  ...
  if (isLoadingAnnonce) {
    ...
    content = (
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        >
          <h2>'Confirm Delete Ad'</h2>
          <button onClick={confirmDeleteImage}>Yes</button>
